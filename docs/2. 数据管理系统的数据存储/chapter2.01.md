...
由此可见，图2.01呈现的金字塔型存储架构是一种能够兼顾存储成本和访问效率的合理结构。它一方面用廉价设备实现数据的持久性存储，另一方面又用高速设备去加速热点数据的访问。
...
**1**. 思考题：由本章内容我们得知，计算机的数据存储体系是一个金字塔结构。该结构有一个明确的规律：越上层的设备存取速度越快，但价格越贵；越下层的设备存取速度越慢，但价格越便宜。假设人类研发出了一种新的存储设备，它比金字塔结构中的某一层设备速度更快、价格更低。当我们将这个新设备放入到存储体系后，它是否会破坏这个金字塔结构的既有规律？为什么？

**2**. 思考题：假设一个很大的整数数组存放在硬盘上（磁盘和固态硬盘均可），数组中的整数是乱序的。现在分别使用归并排序算法和快速排序算法对数组中的整数进行排序。请问哪个算法的速度更快？（提示：虽然每次读写数据都要访问硬盘，但计算机会自动将近期刚访问过的数据块（Block）缓存在内存中，以加速数据的读写。但由于内存有限，一段时间内未被访问的数据块将会从内存缓存中被移除。）
 

[**上一页<<**](../1. 数据管理系统的功能、模型与语言/chapter1.17-G.md) | [**>>下一页**](chapter2.02.md)