...
```SQL
[例3.01] 查询沐辰同学所选课程号以及课程成绩。
SELECT Cno, Grade
FROM Student, SC
WHERE Student.Sno = SC.Sno AND Student.Sname = '沐辰'; 
```
当处理例3.01的SQL语句时，关系数据库系统首先将SQL语句解析为查询树。在查询解析步骤（如图3-2-2所示）中，关系数据库系统首先通过词法分析提取出SQL语句中的关键词、标识、运算符、常量、分隔符等成分，然后通过语法分析将词法分析提取的成分构建为一颗语法分析树，最后根据数据字典的关系定义检查语法分析树的语义正确性，并将其转化为基于关系代数表达式的查询树。
...
关系数据库系统最经典的查询执行模型就是在3.01.4节描述的火山模型。在关系数据库的火山模型中，物理执行计划中的每个关系操作或算子被作为一个数据处理算子（operator）。多个算子被串联成数据处理的流水线。
...
[**上一页<<**](chapter3.01.md) | [**>>下一页**](chapter3.03-R.md)
