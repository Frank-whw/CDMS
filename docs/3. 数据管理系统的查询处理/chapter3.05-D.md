...
## 3.05.1 一阶段查询处理（singe-stage）
```SQL
[例3.06] 查询计算机系所有学生的学号和姓名
db.student.find({
	"department":"计算机"
},
{
...
文档数据库MongoDB通常将find查询看作一阶段查询处理，主要进行文档数据读取、条件过滤和结果返回这一主要过程。当处理例3.06的find查询时，文档数据库系统的查询处理过程如图3-5-1所示。
...
[**上一页<<**](chapter3.04-R.md) | [**>>下一页**](chapter3.06-G.md)
