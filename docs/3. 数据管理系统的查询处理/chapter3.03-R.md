...
索引连接算法是利用连接属性上的索引来实现连接操作。例3.04中，Student表（S表）在Sno上构建了B+树聚集索引，SC表（R表）在Sno和Cno上构建了B+树索引结构。SC表上的索引称为组合索引，该索引的层数为L，索引在Sno上的选择基数为V（平均有V个元组具有相同的Sno值）。
...
[**上一页<<**](chapter3.02-R.md) | [**>>下一页**](chapter3.04-R.md)
