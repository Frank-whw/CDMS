...
在Cypher中，要创建图模型，需要使用`CREATE`关键字。例如，按照图R3.05的图模型，创建一个姓名为“沐辰”的 `Student` 类节点和两个课程名分别为“高数”和“数据库”的 `Course` 类节点，然后在上述学生-课程节点之间分别创建边：
```SQL
...
[**上一页<<**](chapter1.16-G.md) | [**>>下一页**](../2.%20数据管理系统的数据存储/index.md)
